...
Original file is located at
    https://colab.research.google.com/drive/1PdvcZTDBNUkefk-90-E-X2dY7m94BTi3
...
white_like = build_image_like_tensor(320,250,3,255/255)
gray_like = build_image_like_tensor(320,250,3,128/255)
black_like = build_image_like_tensor(320,250,3,0)
...
red_like = build_image_like_tensor(320,250,3,[1,0,0])
green_like = build_image_like_tensor(320,250,3,[0,1,0])
blue_like = build_image_like_tensor(320,250,3,[0,0,1])
...
def normalize_tensor(input_tensor: torch.Tensor) -> torch.Tensor:
    ...
    # YOUR CODE HERE
    #normalized_input = (input_tensor - np.min(input_tensor))/(np.max(input_tensor) - np.min(input_tensor))
    normalized_input = input_tensor/255
...
def convolution_forward_numpy(image, kernel):
    ...
    for k in range(0,I.shape[0]-2) :
        for l in range(0,I.shape[1]-2) :
            img = I[k:k+3,l:l+3]
            temp = 0
            for i in range(0,kernel.shape[0]) :
                for j in range(0,kernel.shape[1]) :
                    temp += np.dot(img[i,j],kernel[i,j])
            R[k,l]=temp
...
# YOUR CODE HERE
kernel = K_1
output_image = convolution_forward_numpy(image[:,:,1], kernel)
